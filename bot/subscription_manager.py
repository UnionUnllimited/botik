...
async def grant_subscription(
    ...
    *,
    ...
    async with user_lock:
        ...
        # Дальше — ветка «завести новую учётку», и роутерному клиенту её
        # заводить нельзя. Срок ему приносит магазин, роутер ходит по своей
        # учётке; эта стала бы второй, телефонной. Бонусные дни легли бы на
        # неё, роутер продолжил бы ходить по прежней, а клиент прочитал бы
        # «+7 дней» и не нашёл их нигде.
        #
        # Проверка именно здесь, а не выше: клиент их бота мог купить роутер,
        # и его собственную телефонную подписку — ветка `xui_client_uuid`
        # выше — продлевать нужно по-прежнему. Роутера она не касается.
        #
        # И не у зовущих: их два десятка — пробные дни, админская правка,
        # реферальный бонус, повтор платежа, — каждый следующий забыли бы.
        if await db_helpers.is_shop_client(user_id):
            logger.warning(
                f"Новая учётка для {user_id} не заводится: это роутерный клиент, "
