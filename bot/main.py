        ...
        if active_sub:
            ...
            await bot.send_message(
                ...
                format_text_template(
                    ...
                    days=days_paid,
                    expiry_date=format_msk_date(expiry_date),
                ),
    ...
    if subscription_data:
        ...
        await bot.send_message(
            ...
            format_text_template(
                ...
                DEFAULT_PAYMENT_SUCCESS,
                days=days_to_add,
                expiry_date=format_msk_date(subscription_data['expiry_date']),
            ),
            reply_markup=keyboards.get_success_with_referral_keyboard()
        )
...
DEFAULT_PAYMENT_SUCCESS = REST_TEXT_DEFAULTS['text_payment_success']
"""Сообщение после оплаты. Умолчание нужно здесь, а не только при
заведении текстов: стёртая настройка отправляла клиенту пустую строку,
а Telegram на пустом сообщении отвечает отказом — деньги взяты, а
подтверждения нет."""
...
async def show_main_menu(message_or_query: Message | CallbackQuery, edit_message: bool = False):
    ...
    if active_sub:
        ...
        # Третье положение — оплаченное сгорело, роутер так и не вышел
        # на связь. Даты нет, как и у ждущего, а значит противоположное:
        # «дни доставки не сгорают» читалось бы прямо против письма,
        # которое клиент получил в день сгорания.
        from src.shop_texts import (
            BURNED,
            WAITING_FOR_ROUTER,
            shop_client_burned,
            shop_client_is_waiting,
        )

        _until = (user_db_data or {}).get('subscription_end_date')
        _state = (user_db_data or {}).get('shop_subscription_status')
        if shop_client_is_waiting(_until, _state):
            text_to_send += "\n\n" + WAITING_FOR_ROUTER
        elif shop_client_burned(_until, _state):
            text_to_send += "\n\n" + BURNED
        else:
            text_to_send += "\n\n" + (
