...
def attach_settings_routes(admin_bp_instance, query_db_func, execute_db_func):
    ...
    # ─── Конфиг страницы подписки (subpage) ─────────────────────────────────

    @admin_bp_instance.route('/settings/subpage', methods=['GET'])
    async def settings_subpage():
        """Управление настройками subpage (страница подписки).

        Раньше всё хранилось только в `.env` subpage. Теперь — JSON в settings,
        откуда subpage может его подтянуть через xuiweb endpoint
        `/api/settings/info` при старте, если в .env бота включён флаг
        `api_custom_settings=true`. На .env остаётся фолбэк для пустых полей
        и для случая, когда API недоступен.

        Шаблон отрисовывает 3 таба:
          - Основная информация (PROJECT_NAME, SUPPORT_URL, и т.п.)
          - Ссылки приложений (Happ × 6 + INCY × 6)
          - HTTP-заголовки RESP_HEADER_* (динамическая таблица ключ/значение)
        """
        from web_admin.run import current_user
        if not current_user.is_admin:
            return '', 403
        from web_admin.core.subpage_config import (
            load_config, ALLOWED_THEMES, KNOWN_APP_USER_AGENTS, LINK_KEYS,
        )
        from web_admin.async_db import async_query_db
        cfg = await load_config()
        # hwid_limit_enabled живёт отдельным ключом в settings (его читает xuiweb
        # при каждом запросе подписки, поэтому он не попадает в subpage_config
        # — туда уезжает только то, что нужно непосредственно subpage'у).
        hwid_limit_row = await async_query_db(
            "SELECT value FROM settings WHERE key = 'hwid_limit_enabled'", (), one=True
        )
        hwid_limit_enabled = (hwid_limit_row['value'] if hwid_limit_row else '0') == '1'
        sub_page_row = await async_query_db(
            "SELECT value FROM settings WHERE key = 'sub_page_url'", (), one=True
        )
        sub_page_url = str(sub_page_row['value'] if sub_page_row else '').strip().rstrip('/')
        # Группируем ссылки по семействам, чтобы шаблону было удобно рисовать.
        link_groups = [
            ('Happ', [
                ('LINK_IOS',         'iOS (App Store, RU)',  'apple'),
                ('LINK_IOS_GLOBAL',  'iOS (App Store, Global)', 'apple'),
                ('LINK_ANDROID',     'Android (Google Play)', 'smartphone'),
                ('LINK_ANDROID_APK', 'Android (APK)',         'package'),
                ('LINK_WINDOWS',     'Windows',               'monitor'),
                ('LINK_MAC',         'macOS',                 'laptop'),
                ('LINK_LINUX',       'Linux',                 'terminal'),
            ]),
            ('INCY', [
                ('LINK_INCY_IOS',         'iOS (App Store)',      'apple'),
                ('LINK_INCY_ANDROID',     'Android (Google Play)', 'smartphone'),
                ('LINK_INCY_ANDROID_APK', 'Android (APK)',         'package'),
                ('LINK_INCY_WINDOWS',     'Windows',               'monitor'),
                ('LINK_INCY_MAC',         'macOS',                 'laptop'),
                ('LINK_INCY_LINUX',       'Linux',                 'terminal'),
            ]),
        ]
        return await render_template(
            'settings_subpage.html',
            cfg=cfg,
            allowed_themes=ALLOWED_THEMES,
            known_uas=KNOWN_APP_USER_AGENTS,
            link_groups=link_groups,
            hwid_limit_enabled=hwid_limit_enabled,
            sub_page_url=sub_page_url,
        )

    @admin_bp_instance.route('/settings/subpage/save', methods=['POST'])
    async def settings_subpage_save():
        """Принимает полный JSON конфига от страницы и сохраняет одним
        ключом в settings. Возвращает финальный (валидированный) конфиг.
        """
        from web_admin.run import current_user
        if not current_user.is_admin:
            return jsonify({'ok': False, 'error': 'forbidden'}), 403
        from web_admin.core.subpage_config import save_config
        from web_admin.async_db import async_execute_db
        try:
        ...
        try:
            ...
            # Отдельный ключ hwid_limit_enabled (живёт вне subpage_config — его
            # читает xuiweb на каждый запрос). Принимаем bool/'1'/'0'.
            if 'hwid_limit_enabled' in payload:
                raw = payload.get('hwid_limit_enabled')
                value = '1' if (raw is True or raw == 1 or str(raw).strip() in ('1', 'true', 'True')) else '0'
                await async_execute_db(
                    "INSERT OR REPLACE INTO settings (key, value, description) VALUES (?, ?, ?)",
                    (
                        'hwid_limit_enabled',
                        value,
                        'Включить лимит устройств по HWID (только SUBPAGE)',
                    ),
                )
            return jsonify({'ok': True, 'cfg': cfg})
        except Exception as e:
            current_app.logger.error(f"settings_subpage_save error: {e}", exc_info=True)
            return jsonify({'ok': False, 'error': str(e)}), 500
