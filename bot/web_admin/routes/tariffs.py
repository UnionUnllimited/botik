...
def attach_tariff_routes(admin_bp_instance, query_db_func, execute_db_func):
    ...
    async def _load_device_upgrade_settings() -> dict:
        rows = await query_db_func(
            "SELECT key, value FROM settings WHERE key IN ({})".format(
                ','.join(['?'] * len(_DEVICE_UPGRADE_KEYS))
            ),
            tuple(_DEVICE_UPGRADE_KEYS.keys()),
        ) or []
        loaded = {dict(r)['key']: dict(r)['value'] for r in rows}
        out = {}
        for key, (default, _desc) in _DEVICE_UPGRADE_KEYS.items():
            out[key] = loaded.get(key, default)
        return out

    @admin_bp_instance.route('/tariffs/device-limits', methods=['GET', 'POST'])
    async def device_limits_settings():
        if request.method == 'POST':
            form = await request.form

            # Парсим значения формы (с валидацией)
            ...
            new_values = {
                'device_upgrade_enabled': '1' if form.get('device_upgrade_enabled') in ('1', 'on', 'true') else '0',
                'device_upgrade_max_limit': str(_to_int(form.get('device_upgrade_max_limit'), 20)),
                'device_upgrade_min_days_left': str(_to_int(form.get('device_upgrade_min_days_left'), 3)),
                'device_upgrade_min_price': str(_to_float(form.get('device_upgrade_min_price'), 30.0)),
                'device_upgrade_price_per_slot_per_day': str(_to_float(form.get('device_upgrade_price_per_slot_per_day'), 5.0)),
            }

            # Сохраняем в БД (UPSERT через INSERT OR REPLACE с сохранением description)
            for key, value in new_values.items():
                _default, description = _DEVICE_UPGRADE_KEYS[key]
                await execute_db_func(
            ...
            # Подсасываем в app_conf, чтобы изменения подхватились без перезапуска
            try:
            ...
            await flash('Настройки расширения лимита устройств сохранены.', 'success')
            return redirect(url_for('admin.device_limits_settings'))

        settings_data = await _load_device_upgrade_settings()
        return await render_template(
            'device_limits_form.html',
