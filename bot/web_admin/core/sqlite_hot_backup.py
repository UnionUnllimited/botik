...
Архив поддерживает несколько БД одновременно (vpn_bot.db + devices/devices.db),
...
def build_backup_zip(
    ...
    *,
    ...
    Args:
        db_path: путь к основной БД (например users.db / vpn_bot.db).
    ...
    и тем же именем класть нельзя (но в нашем случае разные имена: vpn_bot.db,
