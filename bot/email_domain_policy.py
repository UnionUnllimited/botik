"""Белый список доменов email: бот проверяет по нему привязку почты.

Чистая логика без зависимостей от web_admin / aiogram.
Настройки хранятся в settings.website_cabinet_config (JSON).

Переехал из вырезанного `website/`: сайт удалён, а проверка домена осталась —
её вызывают привязка почты в боте и её страница настроек в админке.
"""
