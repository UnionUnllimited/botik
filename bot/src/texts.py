...
TXT_SUPPORT_FALLBACK = (
    "<b>Поддержка</b>\n"
    "Нажмите на строку — она скопируется. Отправьте её в чат поддержки вместе "
    "с вопросом.\n\n"
    "Ваш ID: <code>{user_id}</code>\n"
    "{router_line}"
)
"""MAC стоит рядом с ID: по нему оператор находит и подписку, и заказ, а
клиент ищет его на наклейке под роутером и переписывает с ошибкой. У человека
без роутера `{router_line}` пуст и исчезает целиком — «Ваш MAC-адрес:» без
значения было бы хуже, чем ничего."""
...
REST_TEXTS: list[tuple[str, str, str]] = [
    ...
    (
        "text_support",
        TXT_SUPPORT_FALLBACK,
        "Экран поддержки. Переменные: {user_id}, {router_mac}, {router_line}",
    ),
    (
...
REST_LEGACY_TEXTS: dict[str, str] = {
    ...
    # Прежний экран поддержки: без MAC. Оператор просил его первым же
    # сообщением, и клиент шёл переписывать наклейку под роутером.
    "text_support": (
        "<b>Поддержка</b>\n"
...
REST_REDESIGN_MARK = "ui_redesign_2026_08_rest_v2"
