...
async def show(
    ...
    *,
    ...
    photo: str | None = None,
    drop_incoming: bool = True,
    persist: bool = False,
) -> None:
    """Показать экран: по возможности правкой текущего сообщения.

    `drop_incoming=False` оставляет сообщение пользователя в чате — нужно там,
    где оно само по себе часть переписки (например, обращение в поддержку).
