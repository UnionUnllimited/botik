...
сверяем сумму уведомления с суммой платежа в базе.

**Сумма приходит с комиссией.** PLATEGA берёт её сверху: в `paymentDetails.amount`
лежит списанное с клиента, а не выставленное нами — счёт на 92,99 возвращается
как 100,43. Поэтому сверка требует «не меньше выставленного», а не точного
совпадения: недоплата не зачисляется и зовёт оператора алертом, переплата
зачисляется и пишется в журнал (`payment.amount_above_invoice`). `payment.amount`
при этом не трогаем — комиссия до нас не доходит, и в сверке заказа она нашей
выручкой быть не должна.
