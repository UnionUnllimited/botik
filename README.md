...
`/static` (стили витрины), `/lists` и `/firmware` (их читают сами роутеры),
корневые пути LuCI (`/cgi-bin`, `/luci-static`, `/ubus`, `/panel`) — по ним
проксируется панель роутера — и сам корень: в нём стоит витрина.
...
**Тело запроса не ограничивать.** По умолчанию Caddy предел не ставит, и так
и надо оставить: образ прошивки весит 27–54 МБ и уходит из браузера оператора
одним запросом на `/firmware/upload`. Директива `request_body { max_size … }`,
если её когда-нибудь добавят, должна быть не меньше `APP_FIRMWARE_MAX_BYTES`
(по умолчанию 128 МБ), иначе загрузка обрывается на середине и отличить это
от плохой связи оператор не сможет.
...
curl -fsS https://<домен>/firmware/manifest.json  # манифест обновлений роутеров
```

Адрес манифеста зашит в прошивку открытым текстом и меняться не должен:
роутер ходит по нему раз в сутки и другого способа узнать о новой прошивке
не имеет. Пока ничего не выпущено, он честно отдаёт `{"version": 0, …}`
с пустым списком образов — роутеры читают это и ничего не делают.
