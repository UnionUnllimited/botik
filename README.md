...
Бэкенд и админка сервиса роутеров с подпиской на стабильный доступ
к зарубежным ресурсам: управление парком роутеров, туннели к ним, подписки,
клиенты, заказы и вебхуки оплаты.

Клиентская часть — сайт и бот — вынесена в сторонний продукт в `bot/`,
он ставится отдельным процессом на хосте. Порядок — в `docs/state.md`.
...
За нами остались `/admin` (админка и парк роутеров), `/webhooks` (колбэки
оплаты), `/api` (парк для вкладки в админке бота) и корневые пути LuCI
(`/cgi-bin`, `/luci-static`, `/ubus`) — по ним проксируется панель роутера.

Корень домена нам не нужен: клиентский сайт живёт в стороннем продукте
и ставится отдельно. Для Caddy:
