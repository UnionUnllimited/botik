...
class PlategaSettings(EnvSettings):
    ...
    partner_secret: SecretStr = SecretStr("")
    """Реквизиты бота у провайдера — если он торгует под своим мерчантом.

    Подлинность колбэка подтверждают заголовки, которые провайдер присылает
    обратно, и сверяем мы их со своими. Уведомление бота приходит с его
    реквизитами: наша сверка его не признает, и до пересылки дело
    не дойдёт — мы ответим 401, провайдер несколько раз повторит и бросит.
    Опроса статуса у бота для этого провайдера нет, так что клиент заплатит
    и не получит ничего.

    Заполняется, только когда мерчанты разные. Совпали реквизиты — колбэк
    передаётся боту и ничем больше не становится: нашим платежом он стать
    не может, а бот проверит эти же заголовки сам."""
...
class RemnawaveSettings(EnvSettings):
    ...
    """Имя клиентской учётки в панели.

    Приставка обязательна: ручная активация из админки зовёт учётку иначе,
    и по одному имени она перезаписала бы срок оплаченной подписки клиента.

    MAC подставляется в том же виде, в каком оператор его читает и копирует
    (строчными, через дефис): раньше он шёл слитно и заглавными, и найти
    учётку в панели по MAC с наклейки было нельзя."""
    username_template_no_tg: str = "id{user_id}_{mac}"
    """Для клиентов с сайта: Telegram у них нет, и основной шаблон дал бы
    «tgNone_...» — одно имя всем таким клиентам на одном роутере."""
...
class ListsSettings(EnvSettings):
    ...
    s3_prefix: str = "lists/"
    s3_public_url: str = ""
    """Адрес, по которому файл забирает роутер, если он не совпадает с адресом
    хранилища. Пусто — собираем сами из `s3_endpoint` и бакета, как отдаёт
    публичные объекты Yandex. Заполняется, когда перед бакетом стоит CDN:
    образ прошивки весит до 54 МБ, и парк тянет его разом."""
