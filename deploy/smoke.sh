...
# Манифест обновлений роутеры читают без токена, по адресу, зашитому в прошивку.
# Проверять его нужно именно снаружи: отвечает он всегда, но если прокси
# не отдаёт нам этот путь, парк молча перестаёт обновляться — и узнать об этом
# неоткуда, роутеры о себе не сообщают ничего.
check /firmware/manifest.json 200 "манифест обновлений отдаётся"
# На /metrics видно выручку за сутки, число заказов и размер парка.
# Само приложение отдаёт их без секрета — их спрашивает Prometheus изнутри,
# — а закрывает прокси. Проверять это можно только снаружи.
check /metrics 403 "метрики наружу не отдаются"
...
/lists и /firmware (их читают роутеры)
