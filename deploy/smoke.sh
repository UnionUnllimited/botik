...
# Админка без сессии обязана уводить на форму входа, а не показывать данные.
check /admin/   303 "админка закрыта без входа"
check /admin/fleet 303 "парк закрыт без входа"
...
контейнеру api пути /admin и /webhooks. Корень домена теперь занимает сторонний
сайт из bot/website, наш публичный сайт удалён.
HINT
  else
    cat >&2 <<'HINT'
