...
# Корень — витрина. Отдай прокси корень кому-то ещё, и на месте витрины
# окажется его 404: снаружи это неотличимо от несобранной страницы.
check /         200 "витрина в корне"
check /static/landing.css 200 "стили витрины"
...
контейнеру api корень (витрина), пути /api, /webhooks, /media, /static
и корневые пути панели роутера (/cgi-bin, /luci-static, /ubus). Админка бота
живёт под своим секретным путём и ходит к нам по /api.
HINT
  else
    cat >&2 <<'HINT'
