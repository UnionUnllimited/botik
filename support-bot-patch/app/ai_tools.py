...
async def get_my_router(user_id: int) -> dict:
    ...
    if not items:
        return {
            "has_router": False,
            "note": (
                "В базе за этим Telegram-аккаунтом роутера нет. Это НЕ значит, "
                "что устройства у клиента нет: роутер попадает в базу только "
                "после первого выхода в интернет, поэтому у новых и у "
                "неподключённых роутеров ответ всегда такой. НЕ сообщай это "
                "клиенту и не проси его проверять аккаунт — веди обычную "
