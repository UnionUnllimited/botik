...
ADMIN_ANSWERS = {
    ...
    "qa_install": (
        ...
        "📖 Подробнее: <code>http://atlanta.lan/instruction.html</code>"
    ),
    ...
    "qa_tv": (
        ...
        "Wi-Fi <b>Atlanta-…</b> или кабелем в порт <b>LAN</b>.\n\n"
        "❗️ <b>Не работает YouTube на Samsung или LG?</b> "
        "Это известная особенность, решается одним переключателем:\n\n"
        "1️⃣ Панель <code>http://192.168.14.1</code> → "
        "Расширенные настройки → <b>«Обходы и сервер»</b>\n"
        "2️⃣ Режим YouTube / YT → выберите <b>«Запасной»</b>\n"
        "3️⃣ <b>«Сохранить режим»</b>, подождите 10 секунд\n"
        "4️⃣ Выключите телевизор из розетки на 10 секунд\n\n"
        "\U0001F4A1 Многие телевизоры не видят сеть 5 ГГц — "
        "подключайте к <b>Atlanta-2.4</b>."
    ),
    ...
    "qa_reset": (
        ...
        "• <b>Долгое</b> (3 секунды и дольше) — сбрасываются "
        "только имя и пароль Wi-Fi (станет <b>Atlanta-…</b>, пароль "
        "<code>11111111</code>) и пароль панели (<code>admin</code> / "
        "<code>admin</code>).\n\n"
        "✅ <b>Подписка, обходы и выбранные серверы "
        "сохраняются.</b> Сброс их не трогает.\n\n"
        "⚠️ Единственное, что придётся ввести заново, — логин "
        "и пароль PPPoE, если ваш провайдер их требует. Держите "
        "договор под рукой."
    ),
