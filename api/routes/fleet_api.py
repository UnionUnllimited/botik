...
DEVICE_LABELS = {
    ...
    "active": "работает",
    # «Отвязан», а не «изъят»: состояние ставит кнопка «Отвязать клиента», а
    # роутер при этом остаётся у человека. На складской вкладке их админки то
    # же состояние называется «Отвязано» — одно и то же в двух вкладках одной
    # панели не должно читаться как два разных.
    "revoked": "отвязан от клиента",
...
REBOOT_COMMAND = router_shell.QUICK_COMMANDS["reboot"][1]
"""Перезагрузка идёт по SSH через туннель — другого пути к роутеру у нас нет.

Текст берём из общего списка команд: та же перезагрузка есть у клиента
в приложении, и две копии строки однажды разъедутся."""
